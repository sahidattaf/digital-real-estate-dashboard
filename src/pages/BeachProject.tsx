...
import type { InvestmentStat } from '@/types/project';
...
const project = getProject('beach')!;
const { highlight, investmentData, prospectusUrl, financialModelUrl } = project;
        ...
        <div className="max-w-7xl mx-auto space-y-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                {prospectusUrl && (
                    <a
                        href={prospectusUrl}
                        className="bg-ocean-600 hover:bg-ocean-500 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-lg hover:shadow-ocean-500/30 flex items-center gap-2"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-2 glass-panel p-6 rounded-2xl flex flex-col md:flex-row gap-6 relative overflow-hidden">
                        ...
                        <div className="w-full md:w-1/2 h-64 md:h-auto rounded-xl overflow-hidden relative">
                            ...
                            <div className="absolute top-4 left-4">
                                <Badge variant="default" className="bg-sunset-500 hover:bg-sunset-600">{highlight.badgeLabel}</Badge>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col justify-center relative z-10">
                            ...
                            <h2 className="text-2xl font-bold text-white mb-3">{highlight.title}</h2>
                            <p className="text-slate-400 mb-6 leading-relaxed">{highlight.description}</p>
                            <div className="grid grid-cols-2 gap-4">
                                {highlight.features.map(({ icon: Icon, label }) => (
                ...
                {investmentData && (
                    <div className="glass-panel p-6 rounded-2xl flex flex-col gap-4">
                        ...
                        {investmentData.map((stat: InvestmentStat) => (
                            <AnalysisCard
                                key={stat.title}
                                title={stat.title}
                                value={stat.value}
                        ...
                        {financialModelUrl && (
                            <a
                                ...
                                className="mt-auto w-full py-3 rounded-lg border border-ocean-500/50 text-ocean-400 hover:bg-ocean-600/10 font-medium transition-colors text-center"
                            >
